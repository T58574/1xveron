...
- **Поддержка CLI (`--headless` / `--server-only`)**: Добавлен динамический вызов Win32 API `AttachConsole(ATTACH_PARENT_PROCESS)`. Если бинарник запускается из терминала в режиме сервера, он автоматически цепляется к консоли родительского процесса и выводит логи, не создавая лишних окон.
